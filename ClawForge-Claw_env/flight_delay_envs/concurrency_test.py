...
from claw_envs.flight_delay_envs import FlightDelayEnvironment
...
def run_isolated_session(job: dict[str, Any]) -> dict[str, Any]:
    ...
    env = FlightDelayEnvironment(data_root=job.get("data_root"), state_root=job["state_root"])
    session_id = job["session_id"]
    scenario_id = job["scenario_id"]
    behavior = job.get("behavior", "good")

    try:
        env.create_session(session_id, scenario_id, overwrite=False)

        if behavior == "good":
            env.list_flights(session_id, status="delayed")
            env.detect_delayed_flights(session_id)
            env.list_hotel_bookings(session_id)
            env.list_transport_bookings(session_id)
            env.list_notifications(session_id)
        elif behavior == "partial":
            env.list_flights(session_id)
            env.check_flight_status(session_id, "flight_ua123")
    ...
    try:
        for loop_idx in range(loops):
            if loop_idx % 2 == 0:
                ...
                env.update_flight_status(session_id, "flight_ua123", "delayed", delay_minutes=120)
...
def run_contention_batch(
    *,
    ...
    env = FlightDelayEnvironment(data_root=data_root, state_root=state_root)
    ...
    summary.update(
        {
            ...
            "actual_actions": shared_summary.get("action_count"),
            "overall_score": shared_eval.get("overall_score", shared_eval.get("score")),
            "action_count_match": shared_summary.get("action_count") == expected_actions,
        }
    )
    return {
        "summary": summary,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Concurrent stress test for the flight_delay_envs environment.")
    parser.add_argument("--scenario-id", default="flight_delay_cascade", help="Scenario to execute.")
    parser.add_argument("--data-root", default=None, help="Optional override for the flight delay data directory.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    run_prefix = f"fld-{_utc_stamp()}-{random.randint(1000, 9999)}"
    cleanup_state_root = False
    if args.state_root:
        state_root = Path(args.state_root).resolve()
        state_root.mkdir(parents=True, exist_ok=True)
    else:
        state_root = Path(tempfile.mkdtemp(prefix="flight_delay_concurrency_")).resolve()
