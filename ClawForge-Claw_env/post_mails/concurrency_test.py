...
GOOD_X_POST = "Orbital Note launches June 18 with team workspaces for async briefs. Join the waitlist. {tag}"
GOOD_REDDIT_TITLE = "Orbital Note launches June 18 for async brief teams {tag}"
GOOD_REDDIT_BODY = (
    "Orbital Note launches June 18 with a template gallery for recurring briefs, "
    "team workspaces, and a waitlist for the rollout. {tag}"
)
GOOD_X_REPLY = "Team workspaces are in scope, Android is not at launch, and the waitlist is now open. {tag}"
GOOD_REDDIT_REPLY = "The template gallery is included at launch, and the waitlist is open for the rollout. {tag}"

STALE_X_POST = "Orbital Note launches June 11 and the Android app ships on launch day. {tag}"
...
class WorkerResult:
    ...
    unread_email_count: int | None = None
    agent_post_count: int | None = None
...
def _perform_good_flow(env: PostMailsEnvironment, session_id: str, tag: str) -> dict[str, Any]:
    env.list_emails(session_id, query="approved", unread_only=True)
    env.read_email(session_id, "em_004")
    env.read_attachment(session_id, "att_orbital_brief_v3")
    env.read_email(session_id, "em_005")
    env.read_attachment(session_id, "att_social_guardrails")
    env.list_posts(session_id, needs_response_only=True)

    env.publish_post(
        session_id,
        platform="x",
        content=GOOD_X_POST.format(tag=tag),
    )
    env.publish_post(
        session_id,
        platform="reddit",
        title=GOOD_REDDIT_TITLE.format(tag=tag),
        community="r/productivity",
        content=GOOD_REDDIT_BODY.format(tag=tag),
    )
    env.reply_to_post(
        session_id,
        post_id="x_003",
        content=GOOD_X_REPLY.format(tag=tag),
    )
    env.reply_to_post(
        session_id,
        post_id="rdt_002",
        content=GOOD_REDDIT_REPLY.format(tag=tag),
    )

    return env.evaluate_session(session_id)


def _perform_stale_flow(env: PostMailsEnvironment, session_id: str, tag: str) -> dict[str, Any]:
    env.publish_post(
        session_id,
        platform="x",
        content=STALE_X_POST.format(tag=tag),
    )
    return env.evaluate_session(session_id)


def _perform_partial_flow(env: PostMailsEnvironment, session_id: str, tag: str) -> dict[str, Any]:
    env.read_email(session_id, "em_004")
    env.publish_post(
        session_id,
        platform="x",
...
def _choose_behavior(index: int, behavior: str) -> str:
    if behavior != "mixed":
        return behavior
    table = ("good", "good", "good", "partial", "stale")
    return table[index % len(table)]


def run_isolated_session(job: dict[str, Any]) -> dict[str, Any]:
    started = time.perf_counter()
    env = PostMailsEnvironment(data_root=job.get("data_root"), state_root=job["state_root"])
    ...
    try:
        ...
        if behavior == "good":
            ...
            evaluation = _perform_stale_flow(env, session_id, tag)
        ...
        result = WorkerResult(
            ...
            unread_email_count=summary["unread_email_count"],
            agent_post_count=summary["agent_post_count"],
            agent_reply_count=summary["agent_reply_count"],
        )
...
def run_contention_batch(
    *,
    ...
    env = PostMailsEnvironment(data_root=data_root, state_root=state_root)
    ...
    expected_posts = workers * loops_per_worker
    expected_replies = workers * loops_per_worker
    expected_actions = workers * loops_per_worker * 4
    ...
    summary.update(
        {
            ...
            "actual_posts": shared_summary["agent_post_count"],
            "expected_replies": expected_replies,
            "actual_replies": shared_summary["agent_reply_count"],
            "overall_score": shared_eval["overall_score"],
            "action_count_match": shared_summary["action_count"] == expected_actions,
            "post_count_match": shared_summary["agent_post_count"] == expected_posts,
            "reply_count_match": shared_summary["agent_reply_count"] == expected_replies,
        }
    )
    return {
        "summary": summary,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Concurrent stress test for the post_mails environment.")
    parser.add_argument("--scenario-id", default="orbital_launch", help="Scenario to execute.")
    parser.add_argument("--data-root", default=None, help="Optional override for the post_mails data directory.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    run_prefix = f"pm-{_utc_stamp()}-{random.randint(1000, 9999)}"
    cleanup_state_root = False
    if args.state_root:
        state_root = Path(args.state_root).resolve()
        state_root.mkdir(parents=True, exist_ok=True)
    else:
        state_root = Path(tempfile.mkdtemp(prefix="post_mails_concurrency_")).resolve()
    ...
    try:
        ...
        if args.mode in {"contention", "both"}:
            ...
            if (
                ...
                or not contention_summary["post_count_match"]
                or not contention_summary["reply_count_match"]
            ):
                exit_code = 1
