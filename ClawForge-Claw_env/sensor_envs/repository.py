...
class DatasetRepository:
    def __init__(self, data_root: str | Path | None = None):
        ...
        self.sensors_dir = self.data_root / "sensors"
        self.locations_dir = self.data_root / "locations"
    ...
    def load_sensors(self) -> dict[str, dict[str, Any]]:
        payload = _load_json(self.sensors_dir / "sensors.json")
        return {item["sensor_id"]: item for item in payload["sensors"]}

    def load_locations(self) -> dict[str, dict[str, Any]]:
        payload = _load_json(self.locations_dir / "locations.json")
        return {item["location_id"]: item for item in payload["locations"]}
