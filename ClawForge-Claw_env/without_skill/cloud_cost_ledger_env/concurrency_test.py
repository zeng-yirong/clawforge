...
from without_skill.cloud_cost_ledger_env import CloudCostLedgerEnvironment


TARGET_CLUSTER_IDS = [
    "cluster_retail_core",
    "cluster_ads_ranking",
    "cluster_lakehouse_analytics",
]
CURRENT_CATALOG_ID = "cat_apac_q2_2026_current"
SCENARIO_ID = "cloud_cluster_monthly_cost_q2_2026"
...
def _perform_good_flow(env: CloudCostLedgerEnvironment, session_id: str) -> dict[str, Any]:
    env.list_clusters(session_id, cluster_role="business")
    env.list_ledger_entries(session_id)
    env.list_pricing_catalogs(session_id, current_only=True)
    env.get_pricing_catalog(session_id, CURRENT_CATALOG_ID)
    env.read_attachment(session_id, "cost_accounting_rules.md")
    env.read_attachment(session_id, "report_schema.md")
    for cluster_id in TARGET_CLUSTER_IDS:
        env.aggregate_cluster_usage(session_id, cluster_id)
    env.generate_cost_report(session_id, CURRENT_CATALOG_ID)
    return env.evaluate_session(session_id)


def _perform_partial_flow(env: CloudCostLedgerEnvironment, session_id: str) -> dict[str, Any]:
    env.list_clusters(session_id, cluster_role="business")
    env.list_pricing_catalogs(session_id, current_only=True)
    env.aggregate_cluster_usage(session_id, TARGET_CLUSTER_IDS[0])
    env.generate_cost_report(session_id, CURRENT_CATALOG_ID, cluster_ids=TARGET_CLUSTER_IDS[:2])
    return env.evaluate_session(session_id)
...
def run_isolated_session(job: dict[str, Any]) -> dict[str, Any]:
    ...
    env = CloudCostLedgerEnvironment(data_root=job.get("data_root"), state_root=job["state_root"])
...
def run_contention_worker(job: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        for loop_idx in range(loops):
            if loop_idx % 2 == 0:
                cluster_id = TARGET_CLUSTER_IDS[loop_idx % len(TARGET_CLUSTER_IDS)]
                env.aggregate_cluster_usage(session_id, cluster_id)
            else:
                env.generate_cost_report(session_id, CURRENT_CATALOG_ID)
...
def run_contention_batch(
    *,
    ...
    env = CloudCostLedgerEnvironment(data_root=data_root, state_root=state_root)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Concurrent stress test for the cloud_cost_ledger_env environment.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    run_prefix = f"cloud-cost-{_utc_stamp()}-{random.randint(1000, 9999)}"
    cleanup_state_root = False
    if args.state_root:
        state_root = Path(args.state_root).resolve()
        state_root.mkdir(parents=True, exist_ok=True)
    else:
        state_root = Path(tempfile.mkdtemp(prefix="cloud_cost_ledger_concurrency_")).resolve()
