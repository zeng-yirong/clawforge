...
class File:
    ...
    def __init__(self, name: str, content: str = "") -> None:
        ...
        self._last_modified: datetime.datetime = self._timestamp()

    def _timestamp(self) -> datetime.datetime:
        """
    ...
    def _write(self, new_content: str) -> None:
        ...
        self._last_modified = self._timestamp()
...
class Directory:
    ...
    def _add_file(self, file_name: str, content: str = "") -> Union[None, Dict[str, str]]:
        """
        ...
        if file_name in self.contents:
            return {"error": f"File '{file_name}' already exists in directory '{self.name}'."}
        
        new_file = File(file_name, content)
        self.contents[file_name] = new_file
        return None

    def _add_directory(self, dir_name: str) -> Union[None, Dict[str, str]]:
        """
        ...
        if dir_name in self.contents:
            return {"error": f"Directory '{dir_name}' already exists in directory '{self.name}'."}
...
def serialize_filesystem(obj):
    """
    递归将 Directory 和 File 对象转回字典格式
    """
    if hasattr(obj, 'contents'):
        serialized_contents = {}
        for name, item in obj.contents.items():
            serialized_contents[name] = serialize_filesystem(item)
...
class FileSystem:
    ...
    def _load_scenario(self, scenario: dict, long_context: bool = False) -> None:
        ...
        if "root" in scenario and scenario["root"]:
            root_keys = list(scenario["root"].keys())

            if root_keys:
                root_name = root_keys[0]
                root_data = scenario["root"][root_name]

                root_dir = Directory(root_name, None)

                contents_data = root_data.get("contents", {})
                self.root = self._load_directory(contents_data, root_dir)
    ...
    def get_env_state(self):
        return {
            "root": {
                self.root.name: serialize_filesystem(self.root)
            }
        }
    ...
    def mkdir(self, dir_name: str) -> Union[None, Dict[str, str]]:
        ...
        if not self._validate_file_or_directory_name(dir_name):
            return {
                "error": f"mkdir: cannot create directory '{dir_name}': Invalid file or directory name"
            }
    ...
    def touch(self, file_name: str) -> Union[None, Dict[str, str]]:
        ...
        if not self._validate_file_or_directory_name(file_name):
            return {"error": f"touch: cannot touch '{file_name}': Invalid file or directory name"}
    ...
    def echo(
        ...
        if not self._validate_file_or_directory_name(file_name):
            return {"error": f"echo: cannot write to '{file_name}': Invalid file or directory name"}
        ...
        if file_name:
            if file_name in self._current_dir.contents:
                ...
                if isinstance(item, File):
                    ...
                    return {"error": f"echo: {file_name}: Is a directory"}
            else:
    ...
    def cat(self, file_name: str) -> Dict[str, str]:
        ...
        """
        if not self._validate_file_or_directory_name(file_name):
            return {"error": f"cat: '{file_name}': Invalid file or directory name"}
    ...
    def find(self, path: str = ".", name: Optional[str] = None) -> Dict[str, List[str]]:
        ...
        recursive_search(target_dir, path.rstrip("/") if path != "." else ".")
    ...
    def wc(self, file_name: str, mode: str = "l") -> Dict[str, Union[int, str]]:
        ...
        if file_name in self._current_dir.contents:
            ...
            if isinstance(file, File):
                ...
                return {"error": f"wc: {file_name}: Is a directory"}
    ...
    def sort(self, file_name: str) -> Dict[str, str]:
        ...
        if file_name in self._current_dir.contents:
            ...
            if isinstance(file, File):
                ...
                return {"error": f"sort: {file_name}: Is a directory"}
    ...
    def grep(self, file_name: str, pattern: str) -> Dict[str, List[str]]:
        ...
        if file_name in self._current_dir.contents:
            ...
            if isinstance(file, File):
                ...
                return {"error": f"grep: {file_name}: Is a directory"}
    ...
    def tail(self, file_name: str, lines: int = 10) -> Dict[str, str]:
        ...
        """
        if lines < 0:
            return {"error": f"tail: illegal offset -- {lines}"}
        ...
        if file_name in self._current_dir.contents:
            ...
            if isinstance(file, File):
                ...
                return {"error": f"tail: {file_name}: Is a directory"}
    ...
    def _parse_positions(self, positions: str) -> List[int]:
        ...
        if "," in positions:
            ...
            end = int(end) if end else 999999
...
__TEST_CASES__ = [
    {
        'name': 'Test Case 1: Basic Directory Operations (Normal Path & State Change)',
        'steps': [
            {'expect_success': True, 'tool_call': "env['file_system'].pwd()"},
            {'expect_success': True, 'tool_call': "env['file_system'].mkdir(dir_name='test_dir')"},
            {'expect_success': True, 'tool_call': "env['file_system'].ls(a=False)"},
            {'expect_success': True, 'tool_call': "env['file_system'].cd(folder='test_dir')"},
            {'expect_success': True, 'tool_call': "env['file_system'].pwd()"},
            {'expect_success': True, 'tool_call': "env['file_system'].cd(folder='..')"},
            {'expect_success': True, 'tool_call': "env['file_system'].rmdir(dir_name='test_dir')"},
            {'expect_success': True, 'tool_call': "env['file_system'].ls(a=False)"}
        ]
    },
    {
        'name': 'Test Case 2: Basic File Operations (Normal Path & State Change)',
        'steps': [
            {'expect_success': True, 'tool_call': "env['file_system'].touch(file_name='test_file.txt')"},
            {'expect_success': True, 'tool_call': "env['file_system'].ls(a=False)"},
            {'expect_success': True, 'tool_call': "env['file_system'].echo(content='hello world\\nsecond line', file_name='test_file.txt')"},
            {'expect_success': True, 'tool_call': "env['file_system'].cat(file_name='test_file.txt')"},
            {'expect_success': True, 'tool_call': "env['file_system'].echo(content='terminal output')"},
            {'expect_success': True, 'tool_call': "env['file_system'].rm(file_name='test_file.txt')"}
        ]
    },
    {
        'name': 'Test Case 3: Advanced File Operations (wc, sort, grep, tail)',
        'steps': [
            {'expect_success': True, 'tool_call': "env['file_system'].echo(content='b\\nc\\na', file_name='sort_file.txt')"},
            {'expect_success': True, 'tool_call': "env['file_system'].sort(file_name='sort_file.txt')"},
            {'expect_success': True, 'tool_call': "env['file_system'].wc(file_name='sort_file.txt', mode='l')"},
            {'expect_success': True, 'tool_call': "env['file_system'].wc(file_name='sort_file.txt', mode='w')"},
            {'expect_success': True, 'tool_call': "env['file_system'].wc(file_name='sort_file.txt', mode='c')"},
            {'expect_success': True, 'tool_call': "env['file_system'].grep(file_name='sort_file.txt', pattern='b')"},
            {'expect_success': True, 'tool_call': "env['file_system'].tail(file_name='sort_file.txt', lines=2)"}
        ]
    },
    {
        'name': 'Test Case 4: File Comparison and Movement (diff, mv, cp)',
        'steps': [
            {'expect_success': True, 'tool_call': "env['file_system'].echo(content='line1\\nline2', file_name='file1.txt')"},
            {'expect_success': True, 'tool_call': "env['file_system'].cp(source='file1.txt', destination='file2.txt')"},
            {'expect_success': True, 'tool_call': "env['file_system'].echo(content='line1\\nline3', file_name='file2.txt')"},
            {'expect_success': True, 'tool_call': "env['file_system'].diff(file_name1='file1.txt', file_name2='file2.txt')"},
            {'expect_success': True, 'tool_call': "env['file_system'].mv(source='file1.txt', destination='file3.txt')"},
            {'expect_success': True, 'tool_call': "env['file_system'].ls(a=False)"}
        ]
    },
    {
        'name': 'Test Case 5: System utilities (find, du, get_env_state)',
        'steps': [
            {'expect_success': True, 'tool_call': "env['file_system'].mkdir(dir_name='search_dir')"},
            {'expect_success': True, 'tool_call': "env['file_system'].cd(folder='search_dir')"},
            {'expect_success': True, 'tool_call': "env['file_system'].touch(file_name='target.txt')"},
            {'expect_success': True, 'tool_call': "env['file_system'].cd(folder='..')"},
            {'expect_success': True, 'tool_call': "env['file_system'].find(path='.', name='target.txt')"},
            {'expect_success': True, 'tool_call': "env['file_system'].find(path='.')"},
            {'expect_success': True, 'tool_call': "env['file_system'].du(human_readable=True)"},
            {'expect_success': True, 'tool_call': "env['file_system'].get_env_state()"}
        ]
    },
    {
        'name': 'Test Case 6: Boundary Values (Empty strings, 0, negative numbers, excessively long inputs)',
        'steps': [
            {'expect_success': True, 'tool_call': "env['file_system'].echo(content='', file_name='empty.txt')"},
            {'expect_success': True, 'tool_call': "env['file_system'].tail(file_name='empty.txt', lines=0)"},
            {'expect_success': False, 'tool_call': "env['file_system'].tail(file_name='empty.txt', lines=-1)"},
            {'expect_success': False, 'tool_call': "env['file_system'].mkdir(dir_name='')"},
            {'expect_success': False, 'tool_call': "env['file_system'].touch(file_name='a_very_long_file_name_that_exceeds_normal_length_a_very_long_file_name_that_exceeds_normal_length_a_very_long_file_name_that_exceeds_normal_length_a_very_long_file_name_that_exceeds_normal_length_a_very_long_file_name_that_exceeds_normal_length_a_very_long_file_name_that_exceeds_normal_length')"}
        ]
    },
    {
        'name': 'Test Case 7: Error Paths - Non-existent files/directories',
        'steps': [
            {'expect_success': False, 'tool_call': "env['file_system'].cd(folder='non_existent_dir')"},
            {'expect_success': False, 'tool_call': "env['file_system'].cat(file_name='non_existent_file.txt')"},
            {'expect_success': False, 'tool_call': "env['file_system'].rm(file_name='non_existent_file.txt')"},
            {'expect_success': False, 'tool_call': "env['file_system'].rmdir(dir_name='non_existent_dir')"},
            {'expect_success': False, 'tool_call': "env['file_system'].diff(file_name1='non_existent_file.txt', file_name2='non_existent_file2.txt')"}
        ]
    },
    {
        'name': 'Test Case 8: Error Paths - Invalid parameters and types',
        'steps': [
            {'expect_success': True, 'tool_call': "env['file_system'].touch(file_name='empty2.txt')"},
            {'expect_success': False, 'tool_call': "env['file_system'].wc(file_name='empty2.txt', mode='invalid_mode')"},
            {'expect_success': True, 'tool_call': "env['file_system'].mkdir(dir_name='test_dir2')"},
            {'expect_success': False, 'tool_call': "env['file_system'].rmdir(dir_name='empty2.txt')"},
            {'expect_success': False, 'tool_call': "env['file_system'].cd(folder='empty2.txt')"}
        ]
    },
    {
        'name': 'Test Case 9: Cross-method Workflow (Create -> Read -> Update -> Delete)',
        'steps': [
            {'expect_success': True, 'tool_call': "env['file_system'].touch(file_name='crud.txt')"},
            {'expect_success': True, 'tool_call': "env['file_system'].echo(content='initial', file_name='crud.txt')"},
            {'expect_success': True, 'tool_call': "env['file_system'].cat(file_name='crud.txt')"},
            {'expect_success': True, 'tool_call': "env['file_system'].echo(content='updated', file_name='crud.txt')"},
            {'expect_success': True, 'tool_call': "env['file_system'].cat(file_name='crud.txt')"},
            {'expect_success': True, 'tool_call': "env['file_system'].rm(file_name='crud.txt')"},
            {'expect_success': True, 'tool_call': "env['file_system'].ls(a=False)"}
        ]
    },
    {
        'name': 'Test Case 10: Error Path - Invalid File Operations',
        'steps': [
            {'expect_success': False, 'tool_call': "env['file_system'].cp(source='non_existent.txt', destination='dest.txt')"},
            {'expect_success': False, 'tool_call': "env['file_system'].mv(source='non_existent.txt', destination='dest.txt')"},
            {'expect_success': False, 'tool_call': "env['file_system'].sort(file_name='non_existent.txt')"},
            {'expect_success': False, 'tool_call': "env['file_system'].grep(file_name='non_existent.txt', pattern='test')"},
            {'expect_success': False, 'tool_call': "env['file_system'].echo(content='test', file_name='invalid/path.txt')"}
        ]
    }
]