...
class AirlineAPI:
    ...
    def __init__(self):
        ...
        self.insurance_record: Dict[str, dict]
    ...
    def _load_scenario(self, scenario: dict, long_context: Optional[dict] = None) -> None:
        ...
        self.insurance_record = deepcopy(scenario.get("insurance_record", DEFAULT_STATE_COPY["insurance_record"]))
    ...
    def get_env_state(self) -> dict:
        ...
        return {
            ...
            "insurance_record": self.insurance_record,
        }
    ...
    def book_reservation(
        ...
        # 若购买了保险，生成保险记录并关联到预订
        if insurance == "yes":
            insurance_id = self._generate_unique_payment_id()
            if isinstance(insurance_id, dict) and "error" in insurance_id:
                return {"success": False, "error": insurance_id["error"]}
            insurance_id = f"ins_{insurance_id}"
            insurance_cost = 30 * len(passengers)
            self.insurance_record[insurance_id] = {
    ...
    def cancel_reservation(self, reservation_id: str) -> dict:
        ...
        # 退还保险费用并清理保险记录
        insurance_id = reservation.get("insurance_id")
        if insurance_id and insurance_id in self.insurance_record:
            insurance_info = self.insurance_record[insurance_id]
            insurance_cost = insurance_info.get("insurance_cost", 0)
            if insurance_cost > 0:
                # 退款到第一个有效的非证书支付方式
                refunded = False
                for payment in reservation["payment_history"]:
                    ...
                    if pm_id in user["payment_methods"]:
                        ...
                        if pm["source"] != "certificate":
                            ...
                            refunded = True
                            break
                # 若找不到合适退款方式，保险费不退（降级处理，不阻断取消流程）
            # 标记保险为已取消并移除孤儿记录
            self.insurance_record[insurance_id]["status"] = "cancelled"
            del self.insurance_record[insurance_id]
            reservation["insurance_id"] = None
    ...
    def get_insurance_details(self, insurance_id: str) -> dict:
        """
        Get the details of a purchased insurance policy.

        Args:
            insurance_id (str): The ID of the insurance policy
        Returns:
            insurance (dict): The insurance policy details
        """
        if insurance_id not in self.insurance_record:
            return {"success": False, "error": f"Insurance {insurance_id} not found"}
        return {"success": True, "data": self.insurance_record[insurance_id]}
...
__TEST_CASES__ = [
    {
        ...
        'steps': [
            ...
            {'expect_success': True, 'tool_call': "env['airline'].book_reservation(user_id='sara_doe_496', origin='JFK', destination='LAX', flight_type='direct', cabin='economy', flights=[{'flight_number': 'FL123', 'date': '2024-05-15'}], passengers=[{'first_name': 'Sara', 'last_name': 'Doe'}], payment_methods=[{'payment_id': 'credit_card_123', 'amount': 200}], total_baggages=1, nonfree_baggages=0, insurance='no')"},
    ...
    },
    {
        ...
        'steps': [
            {'expect_success': False, 'tool_call': "env['airline'].book_reservation(user_id='non_existent_user', origin='JFK', destination='LAX', flight_type='direct', cabin='economy', flights=[{'flight_number': 'FL123', 'date': '2024-05-15'}], passengers=[{'first_name': 'Sara', 'last_name': 'Doe'}], payment_methods=[{'payment_id': 'invalid_card', 'amount': 200}], total_baggages=1, nonfree_baggages=0, insurance='no')"}
        ]
    },
    {
    ...
    {
        'name': 'Insurance purchase, retrieval, and cancellation refund workflow',
        'steps': [
            {'expect_success': True, 'tool_call': "env['airline'].book_reservation(user_id='sara_doe_496', origin='JFK', destination='LAX', flight_type='direct', cabin='economy', flights=[{'flight_number': 'FL123', 'date': '2024-05-15'}], passengers=[{'first_name': 'Sara', 'last_name': 'Doe'}], payment_methods=[{'payment_id': 'credit_card_123', 'amount': 230}], total_baggages=0, nonfree_baggages=0, insurance='yes')"},
            {'expect_success': True, 'tool_call': "env['airline'].get_env_state()"},
            {'expect_success': True, 'tool_call': "env['airline'].cancel_reservation(reservation_id='DYNAMIC_ID_FROM_PREV_STEP')"},
            {'expect_success': True, 'tool_call': "env['airline'].get_env_state()"}
        ]
    }
]