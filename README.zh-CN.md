...
flowchart LR
    subgraph ENV["环境合成引擎"]
        SCN["场景收集<br/>41 通用 · 42 Claw"] --> ESP["环境规格"]
        ARC["9 种交互原型"] --> SYN["沙箱合成"]
        ESP --> SYN
        TSP["工具规格"] --> SYN
        SYN --> AUD["质量审计<br/>AST + 运行时"]
        AUD --> LIB["139 个可执行环境"]
    end

    subgraph DATA["拓扑感知的数据生成引擎"]
        EXT["工具抽取"] --> GRP["双层依赖图"]
        GRP --> SMP["带权随机游走采样"]
        SMP --> GEN["意图 + 验证器生成"]
        GEN --> TSK["19,777 个任务"]
...
任务 [`清单`](./ClawForge-Claw_data/_manifest.json) 是任务完备性的唯一依据。除 **991 条完备**条目外，它还记录了 **5 条不完整**的生成条目；下游训练应只消费完备的任务包。
