...
from verl.experimental.agent_loop.agent_loop_claw import AgentLoopBase, AgentLoopOutput
...
def _message_preview(messages: list[dict[str, Any]], limit: int = 3, text_limit: int = 240) -> list[dict[str, str]]:
    preview: list[dict[str, str]] = []
    for message in messages[-limit:]:
        preview.append(
            {
                "role": str(message.get("role", "unknown")),
                "content": str(message.get("content", ""))[:text_limit],
            }
        )
    return preview
...
class UniAgentLoop(AgentLoopBase):
    ...
    def _should_return_rollout_logprobs(self) -> bool:
    ...
    async def _build_empty_agent_output(self, exit_reason: str) -> AgentLoopOutput:
        ...
        return AgentLoopOutput(
            ...
            response_logprobs=[0.0] * dummy_response_length if self._should_return_rollout_logprobs() else None,
            routed_experts=None,
            multi_modal_data={},
            reward_score=0,
            num_turns=0,
    ...
    def _save_interaction_result(self, interaction_result: dict):
        ...
        save_content = {
            ...
            "rollout_cache_summary": {
                "prompt_ids_len": len(interaction_result["rollout_cache"].get("prompt_ids", [])),
                "response_mask_len": len(interaction_result["rollout_cache"].get("response_mask", [])),
                "response_logprobs_len": len(interaction_result["rollout_cache"].get("response_logprobs", [])),
                "extra_fields": interaction_result["rollout_cache"].get("extra_fields", {}),
            },
        }
    ...
    def _init_config(self, sampling_params: dict[str, Any], **kwargs):
        ...
        interaction_config = config_dict.setdefault("interaction", {})
        agent_max_turns = self.config.actor_rollout_ref.rollout.agent.get("max_turns", None)
        if agent_max_turns is not None:
            interaction_config["max_turns"] = agent_max_turns

        rollout_trajectory_dir = self.config.trainer.get("rollout_trajectory_dir", None)
        if rollout_trajectory_dir:
            config_dict["log_dir"] = rollout_trajectory_dir
    ...
    async def convert_to_agent_output(self, interaction_result: dict) -> AgentLoopOutput:
        ...
        if not any(rollout_cache["response_mask"]):
            traj_exit_reasons = [step.exit_reason for step in interaction_result.get("trajectory", [])]
            self.logger.error(
                "No model-generated response tokens were produced by the agent loop. "
                f"trajectory_exit_reasons={traj_exit_reasons} "
                f"response_mask_len={len(rollout_cache['response_mask'])} "
                f"message_preview={_message_preview(interaction_result.get('messages', []))} "
                f"rollout_extra_fields={rollout_cache.get('extra_fields', {})}"
            )
        ...
        if self._should_return_rollout_logprobs() and len(response_logprobs) < len(response_ids):
            missing = len(response_ids) - len(response_logprobs)
            self.logger.warning(
                "Missing rollout logprobs for %d response token(s); padding zeros. "
                "traj_exit_reason=%s response_ids_len=%d collected_logprobs_len=%d",
                missing,
                traj_exit_reason,
                len(response_ids),
                len(response_logprobs),
            )
            response_logprobs = response_logprobs + [0.0] * missing
