...
from .env import ActionIncorrectSyntaxError, ActionPermissionError, ActionTimeoutError, AgentEnv, TerminalNotAliveError
...
from .tool_status import ToolStatus, command_status_from_exit_code
...
class AgentInteraction:
    ...
    async def step(self, step_idx: int):
        ...
        * **Tool**: per-call :class:`ToolResult` on ``step_output.tool_results``
          with ``status`` in ``{ok, timeout, syntax_error, permission_error, command_error, skipped}``.
        * **Step**: ``step_output.exit_reason`` + ``done``:
        ...
        with simple_timer("tool_calls", self.rollout_cache["metrics"]):
            for idx, tool_call in enumerate(tool_calls):
                ...
                status: ToolStatus
                try:
                    action_result = await self.env.run_action_with_status(action_cmd, action_timeout=self.action_timeout)
                    observation = str(action_result["observation"])
                    exit_code = int(action_result["exit_code"])
                    status, mark_finished = command_status_from_exit_code(tool_call.function.name, exit_code)
                    if mark_finished:
                        saw_finish = True
                    elif status == "command_error":
                        self.logger.error(f"Tool command exited with status {exit_code}:\n{observation}")
        ...
        if saw_finish:
            ...
            return step_output
        if any(tr.status in ("timeout", "syntax_error", "permission_error", "command_error") for tr in tool_results):
