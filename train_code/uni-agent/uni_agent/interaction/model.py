...
class AgentChatModel:
    ...
    def __init__(self, **data):
        ...
        if not hasattr(self, "logger"):
            run_id = getattr(self, "run_id", "model")
            self.logger = get_logger("chat-model", run_id=run_id)
    ...
    async def query(
        ...
        **kwargs,
        ...
        # vLLM mutates the sampling params dict in-place (for example by
        # rewriting `logprobs`). Copy per request so later turns in the same
        # trajectory still see the original settings.
        sampling_params = copy.deepcopy(kwargs.get("sampling_params", self.sampling_params) or {})
        ...
        stop_reason = getattr(token_output, "stop_reason", None)
        if len(response_ids) == 0:
            last_message = messages[-1] if messages else {}
            last_role = last_message.get("role", "unknown")
            last_preview = str(last_message.get("content", ""))[:400]
            self.logger.error(
                "Model returned zero tokens. "
                f"request_id={request_id} prompt_len={len(prompt_ids)} stop_reason={stop_reason!r} "
                f"sampling_params={sampling_params} last_message_role={last_role} "
                f"last_message_preview={last_preview!r}"
            )
        elif stop_reason not in (None, "completed"):
            self.logger.warning(
                f"Model generation stop_reason={stop_reason!r} "
                f"request_id={request_id} completion_tokens={len(response_ids)}"
            )
        ...
        token_extra_fields = dict(token_output.extra_fields or {})
        rollout_cache["extra_fields"].update(token_extra_fields)
        max_global_steps = token_extra_fields.get("max_global_steps")
        if max_global_steps is not None:
            rollout_cache["extra_fields"]["max_global_steps"] = max_global_steps
        rollout_cache["extra_fields"]["last_stop_reason"] = stop_reason
        rollout_cache["extra_fields"]["last_completion_tokens"] = len(response_ids)
