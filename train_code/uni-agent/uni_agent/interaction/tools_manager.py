...
class ToolsManager:
    ...
    def get_tool_bash_command(self, tool_call: OpenAIFunctionToolCall) -> str:
        ...
        # Route generic tools through `env` so bash builtins like `read`
        # do not shadow the installed runtime scripts on PATH.
        runtime_name = tool_by_name.get(func_name).runtime_name if func_name in tool_by_name else func_name
        cmd_parts = ["env", shlex.quote(runtime_name)]
