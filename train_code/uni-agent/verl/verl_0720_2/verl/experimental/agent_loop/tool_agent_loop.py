...
from verl.experimental.agent_loop.nanoclaw_support import (
    bool_env,
    inject_nanoclaw_trajectory,
    int_env,
    nanoclaw_apply_posthoc_turn_masks,
    nanoclaw_final_answer,
    nanoclaw_looping_response_repeat_count,
    nanoclaw_record_bad_turn_span,
    write_nanoclaw_trajectory,
)
...
class AgentData:
    ...
    def __init__(
        ...
        self.assistant_response_tokens = 0
...
class ToolAgentLoop(AgentLoopBase):
    ...
    async def run(self, sampling_params: dict[str, Any], **kwargs) -> AgentLoopOutput:
        ...
        tools_kwargs = inject_nanoclaw_trajectory(tools_kwargs, trajectory)
        task_id = str(extra_info.get("task_id") or kwargs.get("data_source") or f"sample_{kwargs.get('index', 'unknown')}")
        rollout_index = int(trajectory.get("rollout_n", 0))
        rollout_step = trajectory.get("step")
        rollout_sample_index = trajectory.get("sample_index")
        rollout_label = f"step_{rollout_step}/{task_id}_sample_{rollout_index}"
        ...
        while state != AgentState.TERMINATED:
            if state == AgentState.PENDING:
                ...
                agent_data.termination_reason = "invalid_state"
        ...
        output.extra_fields.update(
            {
                "rollout_termination_reason": agent_data.termination_reason,
                "rollout_final_answer": nanoclaw_final_answer(agent_data),
                "rollout_assistant_turns": agent_data.assistant_turns,
                "rollout_user_turns": agent_data.user_turns,
                "rollout_tool_call_count": agent_data.tool_call_count,
                "rollout_response_tokens": len(agent_data.response_mask),
                "rollout_assistant_response_tokens": agent_data.assistant_response_tokens,
            }
        )
        elapsed = time.time() - started_at
        nanoclaw_state = agent_data.extra_fields.get("nanoclaw") if isinstance(agent_data.extra_fields, dict) else None
        result_dir = nanoclaw_state.get("result_dir") if isinstance(nanoclaw_state, dict) else None
        try:
            write_nanoclaw_trajectory(agent_data, elapsed=elapsed)
        except Exception as trajectory_exc:
            logger.warning("Failed to write Nanoclaw trajectory: %s", trajectory_exc)
        print(
            "[nanoclaw_rollout] "
            f"{rollout_label} status={agent_data.termination_reason} "
            f"batch_index={rollout_sample_index} "
            f"assistant_turns={agent_data.assistant_turns}/{self.max_assistant_turns or 'unlimited'} "
            f"user_turns={agent_data.user_turns}/{self.max_user_turns or 'unlimited'} "
            f"tool_calls={agent_data.tool_call_count} "
            f"assistant_response_tokens={agent_data.assistant_response_tokens}/"
            f"{self.max_assistant_response_length or 'unlimited'} "
            f"response_tokens={len(agent_data.response_mask)}/{self.response_length} "
            f"dir={result_dir} elapsed={elapsed:.2f}s",
            flush=True,
        )
        return output
    ...
    async def _handle_generating_state(
        ...
        remaining_total_response_tokens = self.response_length - len(agent_data.response_mask)
        if not ignore_termination and remaining_total_response_tokens <= 0:
            agent_data.termination_reason = "max_response_tokens"
            return AgentState.TERMINATED

        if self.max_assistant_response_length is not None:
            remaining_assistant_tokens = self.max_assistant_response_length - agent_data.assistant_response_tokens
            if not ignore_termination and remaining_assistant_tokens <= 0:
                agent_data.termination_reason = "max_assistant_response_tokens"
                return AgentState.TERMINATED
            if remaining_assistant_tokens > 0:
                sampling_params["max_tokens"] = min(
                    int(sampling_params.get("max_tokens", remaining_assistant_tokens)),
                    remaining_assistant_tokens,
                    remaining_total_response_tokens,
                )
        else:
            sampling_params["max_tokens"] = min(
                int(sampling_params.get("max_tokens", remaining_total_response_tokens)),
                remaining_total_response_tokens,
            )
        ...
        # Agent implementations may populate unrelated extra fields before the first
        # model call (Nanoclaw stores rollout labels/task metadata there). Therefore,
        # dict emptiness cannot be used to decide whether generation-version metadata
        # has been initialized. Merge the version range field-by-field and treat model
        # version 0 as a valid value rather than a false-y sentinel.
        output_min_global_steps = output.extra_fields.get("min_global_steps")
        current_min_global_steps = agent_data.extra_fields.get("min_global_steps")
        if output_min_global_steps is not None and (
            current_min_global_steps is None or output_min_global_steps < current_min_global_steps
        ):
            agent_data.extra_fields["min_global_steps"] = output_min_global_steps

        output_max_global_steps = output.extra_fields.get("max_global_steps")
        current_max_global_steps = agent_data.extra_fields.get("max_global_steps")
        if output_max_global_steps is not None and (
            current_max_global_steps is None or output_max_global_steps > current_max_global_steps
        ):
            agent_data.extra_fields["max_global_steps"] = output_max_global_steps

        for key in SPEC_DECODE_EXTRA_KEYS:
            if key in output.extra_fields:
                previous = agent_data.extra_fields.get(key) or 0
                current = output.extra_fields.get(key) or 0
                agent_data.extra_fields[key] = int(previous) + int(current)

        # Preserve any other rollout-provided metadata without overwriting fields
        # intentionally owned by the concrete agent implementation.
        for key, value in output.extra_fields.items():
            if key not in {"min_global_steps", "max_global_steps", *SPEC_DECODE_EXTRA_KEYS}:
                agent_data.extra_fields.setdefault(key, value)

        agent_data.assistant_turns += 1
        agent_data.response_ids = output.token_ids
        assistant_token_count = len(agent_data.response_ids)
        assistant_response_start = len(agent_data.response_mask)
        agent_data.assistant_response_tokens += assistant_token_count
        if self.enable_continuous_token:
        ...
        looping_repeat_count = nanoclaw_looping_response_repeat_count(
            decoded_response,
            max(
                1,
                int_env(
                    "NANOCLAW_LOOPING_RESPONSE_MIN_CHARS",
                    int_env("NANOCLAW_REPEATED_RESPONSE_MIN_CHARS", 200),
                ),
            ),
            max(
                2,
                int_env(
                    "NANOCLAW_LOOPING_RESPONSE_MIN_CONSECUTIVE_REPEATS",
                    int_env("NANOCLAW_REPEATED_RESPONSE_MIN_CONSECUTIVE_REPEATS", 2),
                ),
            ),
        )
        assistant_response_end = assistant_response_start + assistant_token_count
        masked_looping_response = False
        looping_response_mask_candidate = False
        if bool_env("NANOCLAW_MASK_LOOPING_RESPONSES", False) and looping_repeat_count > 0:
            if bool_env("NANOCLAW_MASK_ONLY_POSITIVE_ADVANTAGE", False):
                looping_response_mask_candidate = True
                nanoclaw_record_bad_turn_span(
                    agent_data,
                ...
                masked_looping_response = True
                agent_data.response_mask[assistant_response_start:assistant_response_end] = [0] * assistant_token_count
                agent_data.metrics["nanoclaw_looping_response_masked_turns"] = (
                    agent_data.metrics.get("nanoclaw_looping_response_masked_turns", 0) + 1
                )
                agent_data.metrics["nanoclaw_looping_response_masked_tokens"] = (
                    agent_data.metrics.get("nanoclaw_looping_response_masked_tokens", 0) + assistant_token_count
                )
        agent_data.trajectory_events.append(
            {
                "type": "assistant",
                "assistant_turn": agent_data.assistant_turns,
                "token_count": assistant_token_count,
                "response_start": assistant_response_start,
                "response_end": assistant_response_end,
                "looping_repeat_count": looping_repeat_count,
                "masked_looping_response": masked_looping_response,
                "looping_response_mask_candidate": looping_response_mask_candidate,
        ...
        if (
            ...
            and self.max_assistant_response_length is not None
            and agent_data.assistant_response_tokens >= self.max_assistant_response_length
        ):
            agent_data.termination_reason = "max_assistant_response_tokens"
            return AgentState.TERMINATED
        ...
        if agent_data.tool_calls:
            ...
            agent_data.termination_reason = "completed_no_tool_call"
            return AgentState.TERMINATED
    ...
    async def _handle_processing_tools_state(self, agent_data: AgentData) -> AgentState:
        ...
        for tool_index, (tool_response, tool_reward, tool_result) in enumerate(responses):
        ...
        if self.enable_continuous_token and not new_images_this_turn:
            ...
            if len(response_mask) >= self.response_length:
                agent_data.termination_reason = "max_response_tokens_after_tool"
                return AgentState.TERMINATED
            agent_data.prompt_ids = merge_result.token_ids
            agent_data.response_mask = response_mask
            if agent_data.response_logprobs:
