...
def _normalize_fa_position_ids(position_ids: Optional[torch.Tensor], query_states: torch.Tensor) -> Optional[torch.Tensor]:
    """Normalize Qwen3.5/Qwen-VL multi-axis position ids for FlashAttention.

    Qwen3.5 may carry MRoPE position ids as (3|4, batch, seq) or
    (batch, 3|4, seq). FlashAttention varlen sequence discovery expects one
    position-id axis. Passing all axes makes Ascend CANN sum the same sequence
    lengths multiple times (for example query T=10131 versus seqLen=30393).
    """
    if position_ids is None:
        return None

    query_batch_size = query_states.size(0)
    ...
    if position_ids.dim() == 3:
        ...
        if position_ids.shape[0] in (3, 4) and position_ids.shape[0] != query_batch_size:
            position_ids = position_ids[0].unsqueeze(0)

    if position_ids.shape[-1] != query_length:
        raise RuntimeError(
            "FlashAttention position_ids length mismatch: "
            f"position_ids.shape={tuple(position_ids.shape)}, "
            f"query_states.shape={tuple(query_states.shape)}, "
...
def _ulysses_flash_attention_forward(
    ...
    **kwargs,
    ...
    # Qwen3.5 text position ids may contain 3/4 MRoPE axes. FlashAttention
    # sequence-boundary discovery must receive a single axis.
    query_length = query_states.size(1)
    fa_position_ids = _normalize_fa_position_ids(position_ids, query_states)
