...
def forward_with_torch_backend(
    ...
    **kwargs,
    ...
    if isinstance(vocab_weights, DTensor):
        # With FSDP2 CPUOffloadPolicy, ``full_tensor()`` materializes the
        # unsharded DTensor on CPU.  The fused backend accesses lm_head.weight
        # directly instead of calling ``self.lm_head(...)``, so it does not
        # trigger the module's normal FSDP pre-forward device-staging hook.
        # Explicitly stage the materialized vocabulary weight next to the
        # hidden states, matching the established Qwen3-VL fused backend.
