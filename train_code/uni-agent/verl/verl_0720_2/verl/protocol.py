...
class DataProto:
    ...
    def concat(data: list["DataProto"]) -> "DataProto":
        ...
        non_tensor_batch = {}
        non_tensor_keys = set().union(*(item.non_tensor_batch for item in data))
        for key in non_tensor_keys:
            reference_value = next(item.non_tensor_batch[key] for item in data if key in item.non_tensor_batch)
            values = []
            for item in data:
                if key in item.non_tensor_batch:
                    values.append(item.non_tensor_batch[key])
                else:
                    values.append(np.full((len(item), *reference_value.shape[1:]), None, dtype=object))
