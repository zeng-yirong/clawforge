...
def apply_nanoclaw_positive_advantage_bad_turn_mask(batch: DataProto) -> dict[str, float]:
    """Mask positive-advantage tokens in Nanoclaw turns marked as undesirable.

    The rollout records token spans, but defers masking until after GRPO/GAE
    advantages exist. This preserves negative-advantage learning signals while
    preventing looping or duplicated-result turns from receiving reinforcement.
    """
    if not bool_env("NANOCLAW_MASK_ONLY_POSITIVE_ADVANTAGE", False):
        return {}
    bad_turn_spans = batch.non_tensor_batch.get("nanoclaw_bad_turn_spans")
    if bad_turn_spans is None or "advantages" not in batch.batch or "response_mask" not in batch.batch:
        return {}
    ...
    for row_index, spans in enumerate(bad_turn_spans):
        ...
        for span in spans:
            ...
            try:
                ...
                end = min(response_length, int(span.get("end", 0)))
            except (TypeError, ValueError):
                continue
            if end <= start:
                continue
            current_mask = response_mask[row_index, start:end]
            active_mask = current_mask.bool()
            active_tokens = int(active_mask.sum().detach().item())
            if active_tokens <= 0:
                continue
            candidate_turns += 1
            candidate_tokens += active_tokens
            positive_mask = (advantages[row_index, start:end] > 0) & active_mask
            positive_tokens = int(positive_mask.sum().detach().item())
            if positive_tokens <= 0:
                continue
            updated_slice = current_mask.clone()
            updated_slice[positive_mask] = 0
            response_mask[row_index, start:end] = updated_slice
            masked_turns += 1
            masked_tokens += positive_tokens

    if candidate_turns == 0:
        return {}
    return {
        "nanoclaw/positive_advantage_bad_turn_candidate_turns": float(candidate_turns),
        "nanoclaw/positive_advantage_bad_turn_candidate_tokens": float(candidate_tokens),
        "nanoclaw/positive_advantage_bad_turn_masked_turns": float(masked_turns),
        "nanoclaw/positive_advantage_bad_turn_masked_tokens": float(masked_tokens),
    }
...
class RayPPOTrainer:
    ...
    def _write_generations(inputs, outputs, gts, scores, reward_extra_infos_dict, dump_path, global_steps):
        ...
        sample_dump_dir = os.path.join(dump_path, "by_sample", f"step_{global_steps}")
        ...
        os.makedirs(sample_dump_dir, exist_ok=True)
        with open(filename, "w", encoding="utf-8") as f:
            for i in range(n):
                ...
                task_id = str(entry.get("task_id") or "sample")
                safe_task_id = "".join(
                    ch if ch.isalnum() or ch in ("-", "_", ".") else "_" for ch in task_id
                ).strip("._")
                if not safe_task_id:
                    safe_task_id = "sample"
                sample_filename = os.path.join(sample_dump_dir, f"{i:05d}_{safe_task_id}.json")
                with open(sample_filename, "w", encoding="utf-8") as sample_file:
                    sample_file.write(entry_json + "\n")

        print(f"Dumped generations to {filename} and per-sample traces to {sample_dump_dir}")
    ...
    def _log_rollout_data(
        ...
        with marked_timer("dump_rollout_generations", timing_raw, color="green"):
            decoded_inputs = self.tokenizer.batch_decode(batch.batch["prompts"], skip_special_tokens=True)
            ...
            reward_extra_infos_to_dump.setdefault("decoded_input", decoded_inputs)
            optional_non_tensor_keys = [
                "task_id",
                "task_status",
                "task_error",
                "user_plan",
            ...
            for key in optional_non_tensor_keys:
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        metrics.update(apply_nanoclaw_positive_advantage_bad_turn_mask(batch))
                    # update critic
                    if self.use_critic:
                ...
                # === 新增代码：将 metrics 保存到本地 log.jsonl 文件 ===
                import json
                import os
                base_log_dir = "/opt/huawei/dataset/zyr_yuyin/youshen/verl_0720_2/logs"
                os.makedirs(base_log_dir, exist_ok=True)
                log_file_name = f"{self.config.trainer.experiment_name}.jsonl"
                ...
                clean_metrics['step'] = self.global_steps
                # 写入文件
