...
from verl.experimental.agent_loop.agent_loop_claw import (
    AgentLoopManager,
    AgentLoopOutput,
    AgentLoopWorker,
    get_trajectory_info,
...
class AgentLoopWorkerTQ(AgentLoopWorker):
    ...
    async def _run_prompt(self, prompt: dict, sampling_params: dict, trajectory: dict, trace: bool = False) -> None:
        ...
        try:
            ...
            tasks = []
            for i in range(n):
                session_trajectory = {**trajectory, "rollout_n": i}
                task = asyncio.create_task(
                    self._run_agent_loop(
                        run_sampling_params, trajectory=session_trajectory, trace=trace, session_id=i, **prompt
                    )
                )
                tasks.append(task)
    ...
    async def _agent_loop_postprocess(
        ...
        for i, output in enumerate(outputs):
            ...
            field.update(kwargs)
            field["nanoclaw_bad_turn_spans"] = field["extra_fields"].get("nanoclaw_bad_turn_spans", [])
            ...
            # A trajectory without usable rollout-version metadata can occur when
            # an agent terminates before its first model call. In synchronous PPO,
            # generation for trainer step k uses the weights published at k - 1.
            # Preserve real min/max values when present and use that version as a
            # safe, single-version fallback otherwise.
            fallback_model_version = max(int(kwargs["global_steps"]) - 1, 0)
            min_global_steps = field["extra_fields"].get("min_global_steps")
            max_global_steps = field["extra_fields"].get("max_global_steps")
            if min_global_steps is None:
                min_global_steps = fallback_model_version
            if max_global_steps is None:
                max_global_steps = fallback_model_version
            tags.append(
                {
                    "status": "success",
                    ...
                    "min_global_steps": int(min_global_steps),
