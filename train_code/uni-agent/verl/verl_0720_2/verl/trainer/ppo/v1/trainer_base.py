...
from verl.trainer.ppo.ray_trainer import (
    apply_kl_penalty,
    apply_nanoclaw_positive_advantage_bad_turn_mask,
...
class PPOTrainer(ABC):
    ...
    def _compute_advantage(self, batch: KVBatchMeta, metrics: dict) -> KVBatchMeta:
        ...
        fields = [
            ...
            "ref_log_prob",
            "values",
            "nanoclaw_bad_turn_spans",
        ]
        data = tq.kv_batch_get(keys=batch.keys, partition_id=batch.partition_id, select_fields=fields)

        response_mask = data["response_mask"]
        bad_turn_spans = tu.pop(data, "nanoclaw_bad_turn_spans", None)
        data = DataProto(batch=data.to_padded_tensor())
        data.batch["token_level_scores"] = data.batch["rm_scores"]
        data.non_tensor_batch["uid"] = np.array(data.batch.pop("uid").tolist(), dtype=object)
        if bad_turn_spans is not None:
            bad_turn_spans_array = np.empty(len(bad_turn_spans), dtype=object)
            bad_turn_spans_array[:] = bad_turn_spans
            data.non_tensor_batch["nanoclaw_bad_turn_spans"] = bad_turn_spans_array
        ...
        nanoclaw_mask_metrics = apply_nanoclaw_positive_advantage_bad_turn_mask(data)
        metrics.update(nanoclaw_mask_metrics)
    ...
    def _compute_metrics(self, batch: KVBatchMeta, metrics, timing_raw, global_steps, epoch):
        ...
        # Version metadata is auxiliary and must never terminate a completed PPO
        # update. Older/custom agent outputs and synthetic padding tags may contain
        # None. Resolve those values before NumPy's integer conversion; applying
        # non_padding_mask afterwards is too late because dtype=int rejects None.
        def resolve_model_version(tag: dict, key: str) -> int:
            value = tag.get(key)
            if value is None:
                prompt_step = tag.get("global_steps")
                value = max(int(prompt_step) - 1, 0) if prompt_step is not None else max(global_steps - 1, 0)
            return int(value)

        min_global_steps = np.array(
            [resolve_model_version(tag, "min_global_steps") for tag in batch.tags], dtype=int
        )[non_padding_mask]
        max_global_steps = np.array(
            [resolve_model_version(tag, "max_global_steps") for tag in batch.tags], dtype=int
        )[non_padding_mask]
