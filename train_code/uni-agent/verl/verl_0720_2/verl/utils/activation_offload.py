...
class AsyncDoubleBufferGroupOffloadHandler(SynchronizedGroupOffloadHandler):
    ...
    def tensor_pop(self, tensor_tag, **kwargs):
        ...
        # Normally ``on_group_commit_backward()`` prefetches this group through
        # ``bulk_reload_group()`` before autograd asks for an individual saved
        # tensor. Reentrant checkpoint backward may access ``ctx.saved_tensors``
        # first, however, so the state can still be the offloaded ``(key,
        # shape)`` placeholder here. This is a valid ordering on NPU/FSDP2, not
        # a corrupted state. Fall back to a synchronized on-demand reload and
        # cache the recovered base tensor so aliases do not perform duplicate
        # CPU-to-device copies.
        if isinstance(tensor, tuple):
            key, shape = tensor
            offload_mapping = self.group_offload_mapping.get(group_id)
            assert offload_mapping is not None, f"Missing offload mapping for group {group_id}"
            assert key in offload_mapping, f"Missing offloaded tensor {key} for group {group_id}"

            recovered = offload_mapping[key]
            if isinstance(recovered, tuple):
                # Ensure the asynchronous D2H copy that created the CPU backup
                # has completed before reading that backup on the current
                # stream. Use a blocking reload for this exceptional fallback;
                # the normal path remains asynchronously prefetched.
                get_torch_device().current_stream().wait_stream(self.d2h_stream)
                recovered = SynchronizedGroupOffloadHandler.reload(recovered, non_blocking=False)
                offload_mapping[key] = recovered
            tensor = recovered.view(shape)
    ...
    def bulk_reload_group(self, group_to_reload):
        ...
        with get_torch_device().stream(self.h2d_stream):
            ...
            for key, state in offload_mapping.items():
                # An early checkpoint saved-tensor access may already have
                # synchronously restored and cached this base tensor.
                if isinstance(state, tuple):
