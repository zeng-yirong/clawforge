...
class RLHFDataset(Dataset):
    ...
    def __init__(
        ...
        self.allow_missing_prompt_key = config.get("allow_missing_prompt_key", False)
    ...
    def maybe_filter_out_long_prompts(self, dataframe: datasets.Dataset = None):
        ...
        if self.filter_overlong_prompts:
            ...
            if processor is not None:
                ...
                def doc2len(doc) -> int:
                    if self.allow_missing_prompt_key and (prompt_key not in doc or doc[prompt_key] is None):
                        return 0
                    try:
    ...
    def _build_messages(self, example: dict, key: str):
        ...
        elif self.allow_missing_prompt_key:
            messages = []
        else:
            available_keys = sorted(example.keys())
            raise KeyError(
                f"Prompt key `{key}` not found in dataset row. "
                f"Available keys: {available_keys}. "
                "Set `data.allow_missing_prompt_key=true` if your agent loop builds the prompt dynamically."
            )
