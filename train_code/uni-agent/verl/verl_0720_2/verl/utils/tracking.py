...
class FileLogger:
    def __init__(self, project_name: str, experiment_name: str):
        ...
        append = os.getenv("VERL_FILE_LOGGER_APPEND", "0").strip().lower() in {"1", "true", "yes", "on"}
        self.flat = os.getenv("VERL_FILE_LOGGER_FLAT", "0").strip().lower() in {"1", "true", "yes", "on"}
        self.fp = open(self.filepath, "ab" if append else "wb", buffering=0)
    ...
    def log(self, data, step):
        ...
        self.fp.write(
            orjson.dumps(payload, default=self._json_default, option=orjson.OPT_SERIALIZE_NUMPY) + b"\n"
        )
