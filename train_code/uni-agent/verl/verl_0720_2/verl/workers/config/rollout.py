...
class MultiTurnConfig(BaseConfig):
    ...
    enable: bool = False
    # Global interaction-round budget for source_env_agent. A round is one assistant decision,
    # including a decision that requests one or more tool calls.
    ...
    # Deprecated. source_env_agent now uses max_agent_turns across the full rollout.
    # Kept so existing configurations can still be loaded during migration.
    max_tool_turns_per_step: Optional[int] = None
...
class AgentLoopConfig(BaseConfig):
    ...
    # Optional total step cap consumed by external agent loops such as UniAgentLoop.
    max_turns: Optional[int] = None
    source_env_dir: Optional[str] = None
    env_docs_file: Optional[str] = None
    tool_docs_dir: Optional[str] = None
    source_env_long_context: bool = False
    use_validation_reward: bool = True
