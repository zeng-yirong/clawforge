    ...
    async def generate(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            # setdefault does not replace an existing None value. Some custom
            # rollout adapters preserve a stable extra_fields schema with these
            # keys initialized to None, so fill them explicitly when unset.
            if output.extra_fields.get("min_global_steps") is None:
                output.extra_fields["min_global_steps"] = global_steps
            if output.extra_fields.get("max_global_steps") is None:
                output.extra_fields["max_global_steps"] = global_steps
            return output
        finally:
